...
class ShoppingItemsServices:
    def __init__(
        self,
        shopping_list_repot: ShoppingListRepository,
        shopping_item_repot: ShoppingItemsRepository,
        image_repot : ImageRepository,
        user_repot: UserRepository,
        storage_provider: StorageProvider,
        device_repot: DeviceRepository
    ):
        self.shopping_list_repot = shopping_list_repot
        self.shopping_item_repot = shopping_item_repot
        self.user_repot = user_repot
        self.imageRepo = image_repot
        self.storage_provider = storage_provider
        self.device_repot = device_repot
        
        
    async def search_food_by_name(self,  user_id: str, query: str):
        return await self.shopping_item_repot.search_by_food_name(query, user_id)
    

    async def add_item_to_list(self, list_id: int, user_id: str, payload: CreateShoppingItemDTO):        
        shopping_list = await self.shopping_list_repot.get_by_id(list_id, user_id)
        if not shopping_list:
            raise HTTPException(404, "Liste introuvable")
        
        shopping_items = await self.shopping_item_repot.search_by_food_name_in_list(payload.food_name, user_id, shopping_list.id)
        
        if shopping_items:
            raise HTTPException(409,detail="Cet aliment existe déja dans votre liste actuelle.")
        
        item = await self.shopping_item_repot.create(list_id, user_id, payload)
        ...
        await self.shopping_list_repot.rollback_list_to_unfinished(list_id, user_id)

        total_list_prices = await self.shopping_item_repot.get_total_price(user_id, list_id)
        await self.shopping_list_repot.replace_total_cost(list_id, total_list_prices)
        
        device = await self.device_repot.get_by_user_id(user_id)
        ...
        await send_android_notification(
            ...
            data={"route": f"/shopping-list/{str(list_id)}"}
        )
    ...
    async def get_all_items(self, list_id: int, user_id: str) -> list[dict]:
        ...
        return await self.shopping_item_repot.get_all(shopping_list.id)
        
    async def get_shopping_item_by_id(self, item_id: int, user_id: str):
        return await self.shopping_item_repot.get_by_id(item_id, user_id)
                   
    async def get_available_items_count(self, user_id: str,week_number: int):
        shopping_list = await self.shopping_list_repot.get_list_by_week(week_number, user_id, datetime.now().year)
        
        if not shopping_list:
            return 0
        
        return await self.shopping_item_repot.get_items_count(user_id, shopping_list.id, ShoppingListItemEnum.UNPURCHASED)
    ...
    async def get_available_items(self, user_id: str,week_number: int):
        ...
        return await self.shopping_item_repot.get_items_of_list(user_id, shopping_list.id, ShoppingListItemEnum.UNPURCHASED)

    async def complete_shopping_item(self, item_id: int, user_id: str):   
        shopping_item = await self.shopping_item_repot.complete_item(item_id, user_id)
        
        if not shopping_item:
            raise HTTPException(status_code=400, detail="Impossible de marquer cet aliment comme acheté.")
        
        unpurchased_items_count = await self.shopping_item_repot.get_items_count(user_id, shopping_item.shopping_list_id, ShoppingListItemEnum.UNPURCHASED)
        
        if unpurchased_items_count == 0:
            await self.shopping_list_repot.complete_list(shopping_item.shopping_list_id, user_id)
            
        return shopping_item
    
    async def update_shopping_item(self, item_id: int, user_id: str, payload: UpdateShoppingItemDTO): 
        shopping_item = await self.shopping_item_repot.update(item_id, user_id, payload)
        
        if not shopping_item:
            raise HTTPException(status_code=400, detail="Impossible de mettre a jour cet aliment.")
        
        total_list_prices = await self.shopping_item_repot.get_total_price(user_id, shopping_item.shopping_list_id)
        await self.shopping_list_repot.replace_total_cost(shopping_item.shopping_list_id, total_list_prices)
                
        return shopping_item
    
    
    async def delete_shopping_item(self, item_id: int, user_id: str):   
        shopping_item = await self.shopping_item_repot.get_by_id(item_id, user_id)
        ...
        return await self.shopping_item_repot.delete(item_id, user_id)