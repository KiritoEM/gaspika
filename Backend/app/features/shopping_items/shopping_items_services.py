...
class ShoppingItemsServices:
    ...
    async def add_item_to_list(self, list_id: int, user_id: str, payload: CreateShoppingItemDTO):
        ...
        shopping_items = await self.shoppingItemRepo.get_by_food_name(payload.food_name, user_id)
        ...
        total_list_prices = await self.shoppingItemRepo.get_total_items_price(user_id, list_id)
    ...
    async def update_shopping_item(self, item_id: int, list_id:int, user_id: str, payload: UpdateShoppingItemDTO):
        ...
        shopping_item =  await self.shoppingItemRepo.update(item_id, user.id, shopping_list.id, payload)
        ...
        total_list_prices = await self.shoppingItemRepo.get_total_items_price(user.id, shopping_list.id)
        await self.shoppingListRepo.replace_total_cost(list_id, total_list_prices)
